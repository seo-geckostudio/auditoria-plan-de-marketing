...
echo "VERIFICANDO PASO 06_KEYWORD_MAPPING:\n\n";

// Verificar si existe en pasos_plantilla
$sql = "SELECT id, codigo_paso, nombre FROM pasos_plantilla WHERE codigo_paso = '06_keyword_mapping'";
...
$sql = "SELECT COUNT(*) as count FROM paso_campos WHERE paso_plantilla_id = 7";
$result = $pdo->query($sql);
$count = $result->fetch()['count'];
echo "\nCampos configurados: $count\n";

// Verificar si existe paso en auditoria específica
$sql = "SELECT id, auditoria_id, paso_plantilla_id FROM auditoria_pasos WHERE paso_plantilla_id = 7 AND auditoria_id = 26";
$result = $pdo->query($sql);
